        ...
        struct light_state_t const* state)
{
    ...
    if (blink) {
        if (red) {
	    write_int(RED_DELAYON_FILE, onMS);
	    write_int(RED_DELAYOFF_FILE, offMS);
            write_int(RED_BLINK_FILE, blink); }
        if (green) {
	    write_int(GREEN_DELAYON_FILE, onMS);
	    write_int(GREEN_DELAYOFF_FILE, offMS);
            write_int(GREEN_BLINK_FILE, blink); }
        if (blue) {
	    write_int(BLUE_DELAYON_FILE, onMS);
	    write_int(BLUE_DELAYOFF_FILE, offMS);
